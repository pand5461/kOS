...
function lambert2 {
 // Following: Goodling, A procedure for the solution of Lambert's orbital boundary-value problem, Celestial Mechanics and Astronomy 48 (1990), 145-165
 // Der, The superior Lambert algorithm
  ...
  local tau_me to arccos(s) * constant:degtorad + s * sqrt(s2fm1).
  ...
  until converged {
    ...
    until abs(ratio) < tol or niter > nmax {
      ...
      if abs(U) > utol {
        ...
        set f1  to (3 * x * f0 - 2 * (1 - s3 * x / z)) / U.
        set f2 to (5 * x * f1 + 3 * f0 - 2 * s3 * s2fm1 / z^3) / U.
      }
      else {
        set f0 to 2 / 3 * (1 - s3).
      }
